...
import { getServiceIconUrl, getServiceUrl } from '../../lib/api'
...
function OperationsOverview() {
  ...
  return <section className="system-operations-grid">
    <div className="workspace-panel workspace-core-panel system-infrastructure-panel"><div className="workspace-panel-header"><div><ServerCog className="h-4 w-4" /><span><strong>Infrastructure</strong><small>Private access and identity</small></span></div><span>{infrastructure.filter(service => service.state === 'running').length + Number(Boolean(systemQuery.data?.docker_ok)) + Number(Boolean(systemQuery.data?.tailscale_required && systemQuery.data?.tailscale.connected))}/{infrastructure.length + 1 + Number(Boolean(systemQuery.data?.tailscale_required))}</span></div><div>
      ...
      {systemQuery.data?.tailscale_required && <div className="system-infrastructure-row"><span className="system-infrastructure-icon"><Network /></span><span><strong>Tailscale</strong><small>{systemQuery.data?.tailscale.connected ? `${systemQuery.data.tailscale.serve_ports.length} private routes` : 'Sign-in required'}</small></span><i className={`workspace-dot workspace-dot-${systemQuery.data?.tailscale.connected ? 'running' : 'degraded'}`} /></div>}
      {infrastructure.map(service => <div className="system-infrastructure-row" key={service.id}><span className="workspace-app-mark"><img src={getServiceIconUrl(service.id)} alt="" onError={event => { event.currentTarget.style.display = 'none' }} /><span>{service.icon}</span></span><span><strong>{service.display_name}</strong><small>{service.state === 'running' ? `${service.containers.length} components ready` : service.state}</small></span>{service.launch_available && <a href={getServiceUrl(service)} target="_blank" rel="noreferrer" aria-label={`Open ${service.display_name}`}><ExternalLink /></a>}<i className={`workspace-dot workspace-dot-${service.state}`} /></div>)}
    </div></div>
