...
function ServiceInspector({ service, onOpenSettings }: { service: Service; onOpenSettings: (serviceId: string) => void }) {
  ...
  return <section className="workspace-panel workspace-inspector">
    ...
    <div className="workspace-primary-actions">{service.launch_available ? <a href={getServiceUrl(service)} target="_blank" rel="noreferrer"><ExternalLink />Open app</a> : <button disabled title={service.launch_reason || 'This app cannot be opened yet'}><ExternalLink />{service.state === 'running' ? 'SSO route unavailable' : 'Start required'}</button>}<button onClick={() => onOpenSettings(service.id)}><Settings />Configure</button></div>
    {service.launch_available && <p className="px-3 pt-2 text-[11px] text-unknown">{service.launch_transport === 'tailnet_sso' ? 'Tailnet SSO route' : service.launch_transport === 'tailnet_private' ? 'Private Tailnet API route' : 'Local SSO route'}</p>}
